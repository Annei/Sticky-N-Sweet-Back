	...
	<div class="page-contacts flex column" id="page-contacts">
		...
		<div class="contacts-root box grow flex gaps justify-center" :class="contactsClass">
			<div class="card-base card-shadow--small search-card scrollable only-y">
				...
				<el-button icon="mdi mdi-account-plus" @click="openDialog({})"> Añadir contactos</el-button>
			</div>
			<!--termina cuadro de agregar contacto-->
			<!--dialog que se abre cuando seleccionas un contacto-->
			<div class="contacts-list box grow scrollable only-y">
				<div v-for="c in contactsFiltered" :key="c._id" class="flex contact" @click="openDialog(c)">
					...
					<div class="avatar align-vertical">
						<img :src="'/static/images/users/user-'+c._id+'.jpg'" class="align-vertical-middle" alt="user avatar">
					</div>
					<div class="info box grow flex">
						<div class="name box grow flex column justify-center p-10">
							<div class="fullname fs-18"><strong>{{c.name +' '+ c.lastName}}</strong></div>
		...
		<user-dialog :dialogvisible.sync="dialogvisible" :userdata="userdata" @ActualizarListaContactos='consultaContactos'></user-dialog>
	</div>
</template>
...
export default {
	...
	computed: {
		contactsFiltered() {
			return this.contacts.filter(({name,lastName ,email, phone}) => (name+lastName+email+phone).toString().toLowerCase().indexOf(this.search.toString().toLowerCase()) !== -1)
		},
		contactsClass() {
		...
		consultaContactos()
		{
			...
			setTimeout(function(){
				...
				.then(function(response){
					...
					console.log("self; ",self.contacts)
					//self.userdata = response;
					//console.log('esta; ',self.userdata)
				})
